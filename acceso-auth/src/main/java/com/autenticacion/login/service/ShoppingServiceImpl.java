...
import com.autenticacion.login.dao.ShoppingUsuariosDao;
import com.autenticacion.login.dao.UsuarioDao;
import  com.autenticacion.login.modelo.*;
...
public class ShoppingServiceImpl implements ShoppingService {
	...
	@Override
	public ResponseEntity<String> buscarUsuario(String user, String pwd, String userType) {
	    try {
	        String prefixedUser;

	        if ("SHOPPING".equalsIgnoreCase(userType)) {
	            prefixedUser = "shop:" + user;
	            ...
	            if (usuBloqueo == null) {
	                ...
	                if (usuBloqueo.getState() == 1) {
	                    Authentication autentication = authManager
	                            .authenticate(new UsernamePasswordAuthenticationToken(prefixedUser, pwd));
	            ...
	            if (usuBloqueo == null) {
	                ...
	                if (usuBloqueo.getEstado() == 1) {
	                    Authentication authentication = authManager.authenticate(
	                            new UsernamePasswordAuthenticationToken(prefixedUser, pwd));
	                    ...
	                    String estado = switch (usuBloqueo.getEstado()) {
	                        case 2 -> "INACTIVO";
	                        case 3 -> "ELIMINADO";
	                        case 4 -> "BLOQUEADO";
	                        default -> "DESCONOCIDO";
	                    };
	                    return new ResponseEntity<>("El usuario está " + estado, HttpStatus.BAD_REQUEST);
	                }
	            }
	        } else {
	            // Si userType no es "SHOPPING" ni "ADMIN"
			...
			if (token != null) {
				...
				if (authorities.after(fecha)) {
					...
					if (usuarioresult != null) {
						...
						return new ResponseEntity<String>(com.ayalait.utils.MessageCodeImpl.getMensajeServiceUsuarios("70001"),
								HttpStatus.OK);
					} else {
